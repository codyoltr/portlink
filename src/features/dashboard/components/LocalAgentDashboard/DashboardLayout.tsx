...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children, role }) => {
  ...
  return (
    <div className="flex h-screen w-full bg-slate-50 dark:bg-slate-900 font-display overflow-hidden">
      <aside
        className={`${isSidebarCollapsed ? 'w-24' : 'w-80'} transition-all duration-300 h-full bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 flex flex-col flex-shrink-0 z-40 hidden lg:flex`}
      >
        <div
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
          ...
          <div className="flex items-center gap-4 relative">
            {role === 'agent' && (
              <button
                ...
                className="flex items-center gap-2 bg-primary hover:bg-primary/90 text-white px-5 py-2.5 rounded-xl font-bold transition-all shadow-sm hover:shadow-md"
              >
            ...
            <div className="relative">
              ...
              >
                ...
                <div className="hidden md:flex flex-col items-start min-w-0 text-left">
                  ...
                  <span
                    className={`inline-flex items-center px-1.5 py-0.5 mt-0.5 rounded-md text-[9px] font-black uppercase tracking-widest ${getRoleBadgeColor()}`}
                  >
                    {getRoleDisplayName()}
                  </span>
                </div>
                    ...
                    onClick={() => {
                      ...
                      navigate(
                        role === 'agent'
                          ...
                          : '/dashboard/subcontractor/profile'
                      );
                    }}
        ...
        <div className="flex-1 overflow-auto bg-slate-50 dark:bg-slate-900 p-4 md:p-8 lg:p-12">
          <div className="mx-auto w-full max-w-full 2xl:max-w-[1600px]">{children}</div>
        </div>
      </main>
    </div>
  );
